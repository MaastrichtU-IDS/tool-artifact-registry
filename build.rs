...
fn generate(csv_text: &str, version: &str) -> Result<String, String> {
    ...
    out.push_str(
        "# EDAM topics and data types, bundled so the registry's term pickers work offline.\n\
         ...
         # The data branch types artifacts. The topic branch is kept only so records that already\n\
         # cite an EDAM topic still render a label — software topics come from EuroSciVoc now,\n\
         # because asked to classify this estate EDAM returned the same two generic topics for\n\
         # every tool in it.\n\
         #\n\
    ...
    for row in &rows {
        ...
        // still renders a label, but they are branched away from the `topic` the picker offers,
        // which is EuroSciVoc now.
        let picker_branch = if branch == "topic" { "topic-edam" } else { branch };
        out.push_str(&format!("    tar:conceptBranch \"{picker_branch}\" .\n\n"));
        kept += 1;
    }
    if kept < 100 {
...
fn generate_euroscivoc(csv_text: &str) -> Result<String, String> {
    ...
    for row in &rows {
        ...
        body.push_str(&format!("<{iri}> a skos:Concept ;\n    skos:prefLabel \"{label}\" ;\n"));
        ...
        body.push_str("    tar:conceptBranch \"topic\" .\n\n");
        kept += 1;
    }
    if kept < 100 {
